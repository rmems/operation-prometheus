...
def test_expected_failure_cue_allows_modifiers():
    """"expected test failure" / "expected validation failure" (Codex P2)."""
    from lib.normalize import extract_validation

    for body in (
        "## Validation\n\nObserved the expected test failure for the invalid fixture\n",
        "## Validation\n\nThe expected validation failure was observed\n",
    ):
        events = extract_validation({"pull": {"body": body}, "checks": {}})
        assert events[0]["result"] == "pass", body

    # The bound stops the marker reaching an unrelated failure report.
    for body in (
        "## Validation\n\nWe expected a clean run but saw 3 test failures\n",
        "## Validation\n\nNot expected: the suite failed on main\n",
    ):
        events = extract_validation({"pull": {"body": body}, "checks": {}})
        assert events[0]["result"] == "fail", body


def test_macroscope_note_with_linked_attribution_is_stripped():
    """The attribution renders as `<a ...>Macroscope</a> summarized` (Codex P2)."""
    from lib.bots import strip_bot_boilerplate

    body = (
        "## Validation\n\n- cargo test\n- invalid fixture confirms failure behavior\n\n"
        "> [!NOTE]\n> ### Add SAAQ artifact validation flow\n"
        "> - Adds validate-ingest CLI subcommands.\n"
        '> <sup>Generated by <a href="https://macroscope.com">Macroscope</a> '
        "summarized this PR.</sup>\n\n"
    )
    out = strip_bot_boilerplate(body)
    assert "[!NOTE]" not in out
    assert "validate-ingest" not in out
    assert "invalid fixture confirms failure behavior" in out
