...
|----|-------|--------|--------|--------|
| [#20](https://github.com/rmems/thalamic-relay/pull/20) | feat: integrate GPU safety monitoring into main supervisor loop | gpu-compute | feature | 40 kept / 4 emitted (duplicate aggregate review summary and an orphan "rate scales with step_interval_ms" acknowledgement dropped -- the originating cadence feedback wasn't among the retained signals). Wires `check_safety` into the loop; closes #14 / #12. |
...
| #20 | 40 | 4 | 4 | shortlisted (duplicate aggregate summary + orphan cadence acknowledgement dropped) |
