...
def add_digest_reasons(acc: EvalAcc) -> None:
    ...
    _add_digest_conflict_reasons(acc)
    _load_prior_digest(acc)


def _add_digest_conflict_reasons(acc: EvalAcc) -> None:
    ...
    conflicts = _declaration_map_conflicts(acc.card, acc.manifest, acc.names)
    if len(declared_values) > 1 or conflicts:
...
def _add_pr_reasons(acc: EvalAcc) -> None:
    acc.reasons.extend(
        _pr_inventory_reasons(
            acc.record,
            acc.names,
            (acc.pr_number, acc.pull_requests, acc.repository),
        )
    )
