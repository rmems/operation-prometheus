...
def _oid_values(container: Any, keys: tuple[str, ...]) -> list[str]:
...
def _record_oids(record: dict[str, Any]) -> list[str]:
    oids = _oid_values(record.get("repository"), ("base_oid", "head_oid", "commit_oid"))
        ...
        if isinstance(event, dict):
            oids.extend(
                _oid_values(
                    ...
                    ("base_oid", "head_oid", "commit_oid", "tree_oid"),
                )
            )
    return oids


def _valid_oids(oids: list[str]) -> set[str]:
    return {oid for oid in oids if GIT_OID_RE.fullmatch(oid)}
...
def _evaluate_record(
    ...
    *,
    ...
    if inventory_pr is not None:
        record_oids = _valid_oids(_record_oids(record))
        pr_oids = _valid_oids(
            _oid_values(inventory_pr, ("base_oid", "head_oid", "merge_commit_oid"))
        )
        if not record_oids or not pr_oids or record_oids.isdisjoint(pr_oids):
            reasons.append("snapshot_provenance_missing")
...
def _duplicate_id_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    reasons = sorted(set(list(row.get("reason_codes") or []) + ["source_license_unresolved"]))
...
def _bundle_declaration_errors(report: dict[str, Any], manifest: dict[str, Any]) -> list[str]:
    ...
    if isinstance(listed, list) and listed:
        declared_ids = sorted(
            _text(row.get("id")) for row in listed if isinstance(row, dict) and _text(row.get("id"))
        )
        actual_ids = sorted(
...
def build_license_closure_report(
    ...
    *,
    ...
    evaluated = [
        _duplicate_id_row(row) if id_counts[row["record_id"]] > 1 else row for row in evaluated
    ]
    released = sorted(
